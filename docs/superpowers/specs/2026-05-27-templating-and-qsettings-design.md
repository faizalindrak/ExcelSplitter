...
- Add two template modes:
  - **Use Template File**: use a separate template file.
  - **Use Source as Template**: use the selected source worksheet as the template.
...
### Template Mode: Use Template File
...
### Template Mode: Use Source as Template
...
Generation is blocked if Template Mode is **Use Template File** and any required template column is unmapped.
...
The splitter should accept an explicit `column_mapping` for Use Template File mode. Data written to template columns follows template header order:
...
- `template_mode: str = "template_file"`
- `column_mapping: dict[str, str] | None = None`

For `template_mode == "template_file"`, keep the existing template workbook flow but use explicit mapping instead of relying only on exact header intersection.

For `template_mode == "source_template"`, create output workbooks by copying the selected worksheet and deleting rows that do not belong to the current key group.

### Error Handling

- Invalid source/template paths continue to show blocking errors.
- Missing mappings in Use Template File mode raise a clear `ValueError` listing unmapped template columns.
- Unsupported template mode raises `ValueError`.
- Use Source as Template mode should log that separate template path is ignored.
...
- Use Template File output uses manual mapping when template/source headers differ.
- Use Source as Template output preserves only the selected worksheet and only rows for one key.
- QSettings load/save uses Qt settings rather than `.ini` files.
- UI smoke still constructs successfully.

## Acceptance Criteria

- User can select **Use Template File** or **Use Source as Template**.
- Use Template File mode auto-maps matching headers and blocks generation until all required template mappings are filled.
- Use Source as Template mode splits the selected worksheet directly and does not require a template file.
